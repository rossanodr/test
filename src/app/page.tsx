...
import { fetchSTFJurisprudencias } from "./function";
import { Registro, Root } from "../../next-env";
import data from "./data/1.json";

export default function Home() {
  const [search, setSearch] = useState("");
  async function searchj() {
    const stfJurisprudencias = await fetchSTFJurisprudencias(search);
    console.log(stfJurisprudencias);
  }
  const typedData: Root = data as Root; // Realiza a tipagem dos dados importados
    ...
    <main className="flex min-h-screen flex-col items-center justify-between p-24">
      ...
      {typedData.registros.map((registro: Registro) => (
        <div key={registro.registro.id}>
          ...
          <div dangerouslySetInnerHTML={{ __html: registro.registro.txtEmentaHighlight }} />
          ...
          <div dangerouslySetInnerHTML={{ __html: registro.registro.ementaHtml}} />
          ...
          <p>Texto Inteiro Teor Não Analisado: {registro.registro.txtInteiroTeorNaoAnalisado}</p>
          <div dangerouslySetInnerHTML={{ __html: registro.registro.inteiroTeorHtml}} />
          <p>Inteiro Teor (RTF): {registro.registro.inteiroTeorRtf}</p>
          <p>Dispositivo: {registro.registro.dispositivo}</p>
          <p>Dispositivo Não Analisado: {registro.registro.dispositivoNaoAnalisado}</p>
          <p>Numeracao Unica - Número: {registro.registro.numeracaoUnica.numero}</p>
          <p>Numeracao Unica - Dígito: {registro.registro.numeracaoUnica.digito}</p>
          <p>Numeracao Unica - Ano: {registro.registro.numeracaoUnica.ano}</p>
          <p>Numeracao Unica - Orgão: {registro.registro.numeracaoUnica.orgao}</p>
          <p>Numeracao Unica - Tribunal: {registro.registro.numeracaoUnica.tribunal}</p>
          <p>Numeracao Unica - Vara: {registro.registro.numeracaoUnica.vara}</p>
          <p>Numeracao Anterior - Número: {registro.registro.numeracaoAnterior?.numero}</p>
          <p>Numeracao Anterior - Ano: {registro.registro.numeracaoAnterior?.ano}</p>
          <p>Numeracao Anterior - Vara: {registro.registro.numeracaoAnterior?.vara}</p>
          <p>Numeracao Anterior - Tribunal: {registro.registro.numeracaoAnterior?.tribunal}</p>
          <p>Numeracao Anterior - Número Sequencial: {registro.registro.numeracaoAnterior?.numeroSequencial}</p>
          <p>Número Processo Documento: {registro.registro.numProcDocumento}</p>
          ...
          <p>Número Formatado Anterior: {registro.registro.numFormatadoAnt}</p>
        </div>
      ))}
    </div>
    </main>
